 ...
 * ! Revision 2,2.8  02/12/2016
 * ! Revision 2.2.7  01/28/2016
...
int preliminaryRegistration(BASE_LANDSAT  *wbase, WARP_LANDSAT  *wwarp)
{
  ...
  sprintf(cbase.fileName, "%s.working_base_coarse", tempnam(".", "temp_"));
  if(makeCoarse(&(wbase->lnd), &cbase) == FAILURE) {
    fprintf(stderr, "Make coarse resolution file %s error!\n", cbase.fileName);
    return FAILURE;
  }

  sprintf(cwarp.fileName, "%s.working_warp_coarse.%s", tempnam(".", "temp_"));
...
int  preciseRegistrationWithSampling (BASE_LANDSAT  *wbase, WARP_LANDSAT  *warp, WARP_LANDSAT  *temp_warp, WARP_LANDSAT  *wwarp, OUT_LANDSAT *out, OUT_LANDSAT *wout, IN_PARAMETERS *inPars)
{
  ...
  */
  ...
  if(strcasecmp(warp->resampleMethod, "none")==0) return SUCCESS;
  ...
  if(out->proj.sys == UTM) 
    printf("\n\tUTM zone number = %d", out->proj.utm_zone);
  else
    printf("\n\tGCTP_proj_code = %d", out->proj.sys); 
...
int searchCPs(LANDSAT *base, LANDSAT *warp, float **match_p) 
{
  ...
  uint8 *buf8;
  int16 *buf, **bdata, **sdata;
  int16 **base_image, **warp_image;
  int **r_bdata, **r_sdata;
  ...
  alloc_1dim_contig ((void **) (&buf8), MAX_NCOLS, sizeof(uint8));
  alloc_1dim_contig ((void **) (&buf), MAX_NCOLS, sizeof(int16));
  alloc_2dim_contig((void ***) (&bdata), CP_KEYS.CHIP_SIZE, CP_KEYS.CHIP_SIZE, sizeof(int16));
  alloc_2dim_contig((void ***) (&sdata), CP_KEYS.MAX_SIZE, CP_KEYS.MAX_SIZE, sizeof(int16));
  ...
  alloc_2dim_contig((void ***) (&base_image), base->nrows, base->ncols, sizeof(int16));
  alloc_2dim_contig((void ***) (&warp_image), warp->nrows, warp->ncols, sizeof(int16));
  ...
  for(irow=0; irow<warp->nrows; irow++) {
    if(strcasecmp(warp->fileType, "GEOTIFF")==0) {
      ...
      if(warp->nbyte == 1) {
	if (!TIFFReadScanline(warp->fp_tiff, buf8, irow, 0)) {
	  fprintf(stderr, "Read line %d error\n", irow);
	  return FAILURE;
	} 
	for(icol=0; icol<warp->ncols; icol++) buf[icol] = buf8[icol];
      }
      else 
	if (!TIFFReadScanline(warp->fp_tiff, buf, irow, 0)) {
      ...
      if(warp->nbyte == 1) {
	fread(buf8, sizeof(uint8), warp->ncols, warp->fp);
	for(icol=0; icol<warp->ncols; icol++) buf[icol] = buf8[icol];
      }
      else
	fread(buf, sizeof(uint16), warp->ncols, warp->fp);
    }
    for(icol=0; icol<warp->ncols; icol++) warp_image[irow][icol] = buf[icol];
  }
  ...
  for(irow=0; irow<base->nrows; irow++) {
    if(strcasecmp(base->fileType, "GEOTIFF")==0) {
      /* read data array from GeoTiff file */
      if(base->nbyte == 1) {
	if (!TIFFReadScanline(base->fp_tiff, buf8, irow, 0)) {
	  fprintf(stderr, "Read line %d error\n", irow);
	  return FAILURE;
	} 
	for(icol=0; icol<warp->ncols; icol++) buf[icol] = buf8[icol];
	...
	if (!TIFFReadScanline(base->fp_tiff, buf, irow, 0)) {
      ...
      /* read data array from binary file */
      if(base->nbyte == 1) {
	fread(buf8, sizeof(uint8), base->ncols, base->fp);
	for(icol=0; icol<warp->ncols; icol++) buf[icol] = buf8[icol];
      }
      else 
	fread(buf, sizeof(uint16), base->ncols, base->fp);
    }
...
void redoPreciseGeoCorrection(float **match_p, int tnum, LANDSAT *out)
{
  ...
  do {
	...
	printf("%f ", b[i]);
	 ...
	 Feng (10/2008)
      */
  } while((rmse>CP_KEYS.MAX_RMSE || max_dis>1 || fabs(pre_rmse-rmse)>CP_KEYS.DIFF_RMSE) && num>npars+2); 
  

  //#ifdef REG_DEBUG
  //fprintf(log_fp, "\n\n=== Filtered Control Points (Final Selection) ===\n");
  fprintf(log_fp, "cp_no: (base_x, base_y) (warp_x', warp_y') (ploy_x', ploy_y') (res_x, res_y) res_dis\n");
  j = 1;
  for(i=0; i<tnum; i++) 
    if(fabs(match_p[i][4]-1)<0.0000001) { 
      err_x = match_p[i][5] -  match_p[i][2];
      err_y = match_p[i][6] -  match_p[i][3];
      err_dis = sqrt(err_x*err_x + err_y*err_y);
      fprintf(log_fp, "%5d: (%6.1f, %6.1f) (%7.2f, %7.2f) (%7.2f, %7.2f) (%5.2f, %5.2f)  %4.2f\n", j,
	     match_p[i][0], match_p[i][1], match_p[i][2], match_p[i][3], match_p[i][5], match_p[i][6], 
	     err_x, err_y, err_dis);
      j++;
    }
  ...
  if(out->porder == 1) {
    ...
    printf("\n\t\tTotal_Control_Points=%d,  Used_Control_Points=%d, RMSE=%6.3f", tnum, num, rmse);
    printf("\n\t\tx' = %9.3f + %9.6f * x + %9.6f * y", a[0], a[1], a[2]);
    printf("\n\t\ty' = %9.3f + %9.6f * x + %9.6f * y", b[0], b[1], b[2]);
    printf("\n\t\tnote (x, y) is base image location and (x', y') is the resmaple location in warp image\n");
    // fprintf(log_fp, "\nTrying 1st order polynomial transformation");
    fprintf(log_fp, "\nTotal_Control_Points=%d,  Used_Control_Points=%d, RMSE=%6.3f", tnum, num, rmse);
    fprintf(log_fp, "\n\nx' = %9.3f + %9.6f * x + %9.6f * y", a[0], a[1], a[2]);
    fprintf(log_fp, "\ny' = %9.3f + %9.6f * x + %9.6f * y", b[0], b[1], b[2]);
    ...
    printf("\n\t\tTotal_Control_Points=%d,  Used_Control_Points=%d, RMSE=%6.3f", tnum, num, rmse); 
    printf("\n\t\tx' = %9.3f + %9.6f * x + %9.6f * y + %9.6f * x^2 + %9.6f * xy + %9.6f *y^2", 
	   a[0], a[1], a[2], a[3], a[4], a[5]);
    printf("\n\t\ty' = %9.3f + %9.6f * x + %9.6f * y + %9.6f * x^2 + %9.6f * xy + %9.6f *y^2", 
