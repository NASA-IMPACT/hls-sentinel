 ...
 *
 * ! Credits
 *   Feng Gao (Feng.Gao@ars.usda.gov), Developer
 *   Jeff Masek (Jeffrey.G.Masek@nasa.gov)
 *   Robert Wolfe (Robert.E.Wolfe@nasa.gov)
 *
 * ! Revision 2.3.0  4/26/2016
 * ! Revision 2.2.9  2/26/2016
 * ! Revision 2.2.8  2/16/2016
 * ! Revision 2.2.7  1/28/2016
...
int getInParameter(IN_PARAMETERS *inPars, int argc, char *argv[])
{
  ...
  // inPars->data_type = 1; 
  inPars->base_datum = 12;  /* base default in WGS84 */
  inPars->base_nbyte = 1; /* default one byte per pixel */
  inPars->base_sys = -1;   
  ...
  strcpy(inPars->cp_log_file, "cp_log.txt");
  ...
  while(fgets(buffer, MAX_STRLEN, in) != NULL) {
	...
	inPars->base_utm_zone = atoi(tokenptr);
      else if(strcasecmp(label, "BASE_PROJECTION_CODE")==0)
	inPars->base_sys = atoi(tokenptr);
      else if(strcasecmp(label, "BASE_PROJECTION_PARAM")==0)
	for(i=0; i<15; i++) {
	  inPars->base_param[i] = atof(tokenptr);
	  tokenptr = strtok(NULL, seperator);
	}
      else if(strcasecmp(label, "BASE_UNIT")==0)
	inPars->base_unit = atoi(tokenptr);    
      else if(strcasecmp(label, "BASE_DATUM") == 0) {
	inPars->base_datum = atoi(tokenptr);
	if(inPars->base_datum < 0) inPars->base_datum = 12;
      }
      else if(strcasecmp(label, "BASE_SATELLITE")==0)
	sscanf(tokenptr, "%s", inPars->base_satellite);
      else if(strcasecmp(label, "BASE_BAND_DATA_TYPE") == 0 || strcasecmp(label, "BASE_DATA_TYPE") == 0)
	sscanf(tokenptr, "%d", &(inPars->base_nbyte));
	...
	if(inPars->warp_datum < 0) inPars->warp_datum = 12;
      }
	...
	if(inPars->dem_datum < 0) inPars->dem_datum = 12; 
      }
	...
	inPars->porder = atoi(tokenptr);      
      else if(strcasecmp(label, "CP_LOG_FILE")==0)
	sscanf(tokenptr, "%s", inPars->cp_log_file);
  ...
  if(!(inPars->warp_sys == UTM && inPars->warp_utm_zone == inPars->base_utm_zone && inPars->warp_datum == inPars->base_datum))
    inPars->projection_flag = 1;

  inPars->projection_dem_flag = 0;
  if(!(inPars->dem_sys == UTM && inPars->dem_utm_zone == inPars->base_utm_zone && inPars->dem_datum == inPars->base_datum))
  ...
  if((log_fp=fopen(inPars->cp_log_file, "w"))==NULL) {
    fprintf(stderr, "Can't open log file %s for control points\n", inPars->cp_log_file);
    return FAILURE;
  }
...
int getSRTMMetaInfo(DEM *sdem, IN_PARAMETERS *inPars) {
  ...
  if(inPars->dem_sys != -1) {
    ...
    sdem->proj.sys = UTM;
    if(inPars->dem_utm_zone == 999)
      sdem->proj.utm_zone = inPars->base_utm_zone;
    else
      sdem->proj.utm_zone = inPars->dem_utm_zone;
  }
...
int getLandsatMetaInfo(IN_PARAMETERS *inPars, BASE_LANDSAT *base, WARP_LANDSAT *warp, OUT_LANDSAT *out) 
{
  ...
  if(inPars->base_sys != -1) {
    /* if defined new projection system */
    base->proj.sys = inPars->base_sys;
    base->proj.utm_zone = inPars->base_utm_zone;
    base->proj.unit = inPars->base_unit;
    base->proj.datum = inPars->base_datum;
    for(i=0; i<15; i++)
      base->proj.param[i] = inPars->base_param[i];
  }
  else {
    base->proj.sys = UTM;
    base->proj.utm_zone = inPars->base_utm_zone;
    ...
    base->proj.unit = 2;
    for(i=0; i<15; i++)
      base->proj.param[i] = 0.0;
  }
  
  // use defined data type from input (1/2016)
  // base->lnd.nbyte = inPars->data_type;
  base->lnd.nbyte = inPars->base_nbyte;
  if(base->lnd.nbyte == 1)
    base->lnd.satValue = inPars->satValue;
  if(base->lnd.nbyte == 2)
    base->lnd.satValue = inPars->satValue2; 
  ...
  if(strcasecmp(base->lnd.fileType, "GEOTIFF")==0) {
    ...
    // use UTM zone in GeoTIFF file if defined (1/2016)
    /* if (GTIFKeyGet(gtif, GTCitationGeoKey, proj_info, 0, 33) != 0) {
      sscanf(proj_info, "%s %s %d", str, str, &( base->utm_zone));  
      if(strstr(proj_info, "Northern Hemisphere") == NULL)
	base->utm_zone = -1 *  base->utm_zone;
      inPars->utm_zone = base->utm_zone;
      } */
  ...
  if(inPars->warp_sys != -1) {
    ...
    warp->proj.sys = UTM;
    if(inPars->warp_utm_zone == 999)
      warp->proj.utm_zone = inPars->base_utm_zone;
    else
      warp->proj.utm_zone = inPars->warp_utm_zone;
    warp->proj.unit = inPars->warp_unit;;
    warp->proj.datum = inPars->warp_datum;
    for(i=0; i<15; i++)
      warp->proj.param[i] = 0.0;
  }

  warp->lnd.fillValue = inPars->bkValue;
  warp->lnd.satValue = inPars->satValue;
  // warp->lnd.nbyte = inPars->data_type;
  /* find the warp image index from input (1/2016) */
  for(i=0; i< warp->nbands; i++) 
    if(strcmp(inPars->warp_match_fileName, inPars->warp_fileName[i]) == 0) break;
  warp->lnd.nbyte = warp->nbyte[i];
  ...
  if(strcasecmp(warp->lnd.fileType, "GEOTIFF")==0) {
    ...
    // use UTM zone in GeoTIFF file if defined (1/2016)
    /*if (GTIFKeyGet(gtif, GTCitationGeoKey, proj_info, 0, 33) != 0) {
      sscanf(proj_info, "%s %s %d %s", str, str, &( warp->proj.utm_zone), str);  
      if((strstr(proj_info, "Northern Hemisphere") == NULL) && (strcmp(str, "N") != 0))
	warp->proj.utm_zone = -1 *  warp->proj.utm_zone;
      inPars->warp_utm_zone = warp->proj.utm_zone;
      }*/
  ...
  if(!(inPars->warp_sys==UTM && warp->proj.utm_zone==base->proj.utm_zone && warp->proj.datum==base->proj.datum))
    inPars->projection_flag = 1;
  ...
  if(inPars->option!=VERIFY_ONLY) {
    ...
    out->lnd.nbyte = warp->lnd.nbyte;

    out->proj.sys = base->proj.sys;
    out->proj.utm_zone = base->proj.utm_zone;
    out->proj.unit = base->proj.unit; 
    out->proj.datum = base->proj.datum;
    for(i=0; i<15; i++)
      out->proj.param[i] = base->proj.param[i];

    if(strcasecmp(inPars->resampleMethod, "none") !=0 ) 
  ...
  /* for BASE */
  insys = base->proj.sys;
  inzone = base->proj.utm_zone;
  for(i=0; i<15; i++) inparm[i] = base->proj.param[i];
  inunit = base->proj.unit;
  indatum =  base->proj.datum;
  ...
  if(min_rc/100.0 < CP_KEYS.CP_SEED_WIN)
...
int convertCoor(BASE_LANDSAT *base, double *bx, double *by, PROJECTION *proj, double *wx, double *wy, int ptype)
{
  ...
  outsys =  base->proj.sys;
  outzone = base->proj.utm_zone;
  for(i=0; i<15; i++) outparm[i] = base->proj.param[i];
  outunit = base->proj.unit;   /* in meter: 2 */
  outdatum = base->proj.datum; 
...
void getSatMajorAxis(char *satellite, LANDSAT *lnd)
{
  ...
  else if(strcasecmp(satellite, "Sentinel2")==0)
    lnd->satMajorAxis = Sentinel2;
  else if(strcasecmp(satellite, "MySensor")==0)
    lnd->satMajorAxis = MySensor;
  else {
    printf("\nDidn't find any matching satellite for %s, use Landsat8 instead %d\n", 
	   satellite, Landsat8);
    lnd->satMajorAxis = Landsat8;
  }
}
		 ...
		 IN_PARAMETERS *inPars)
{
  ...
  sprintf(wbase->lnd.fileName, "%s.working_base", tempnam(".", "temp_"));
  /*wbase->proj.utm_zone = base->proj.utm_zone;
    wbase->proj.datum = base->proj.datum;*/
  wbase->proj.sys = base->proj.sys;
  wbase->proj.utm_zone = base->proj.utm_zone;
  wbase->proj.unit = base->proj.unit; 
  wbase->proj.datum = base->proj.datum;
  for(i=0; i<15; i++)
    wbase->proj.param[i] = base->proj.param[i];


  /* for warp file (need only one band for working file) */
  copyLandsat(&(wwarp->lnd), &(warp->lnd));
  sprintf(wwarp->lnd.fileName, "%s.working_warp", tempnam(".", "temp_"));
  wwarp->nbands = 1;
  wwarp->nbyte[0] = 2;
  ...
  wout->nbands = 1;
  wout->nbyte[0] = 2;
  sprintf(wout->fileName[0], "%s.working_out", tempnam(".", "temp_"));
...
int rotateWarp(WARP_LANDSAT *warp)
{
  ...
  alloc_1dim_contig ((void **) (&fbuf8), warp->lnd.ncols, sizeof(uint8));
  alloc_1dim_contig ((void **) (&fbuf), warp->lnd.ncols, sizeof(int16));
  alloc_2dim_contig ((void ***) (&data), warp->lnd.nrows, warp->lnd.ncols, sizeof(int16));
  ...
  for(k=0; k<warp->nbands; k++) {
    ...
    sprintf(temp_name, "%s.rotated_warp.b%d", tempnam(".", "temp_"), k+1);
	...
	/* read data array from GeoTiff file */
	if(warp->nbyte[k] == 1) {
	  if (!TIFFReadScanline(warp->fp_tiff[k], fbuf8, irow, 0)) {
	    fprintf(stderr, "Read line %d error\n", irow);
	    return FAILURE;
	  } 
	...
	/* read data array from binary file */
	if(warp->nbyte[k] == 1) {
	  fread(fbuf8, warp->nbyte[k], warp->lnd.ncols, warp->fp[k]);
	  for(icol=0; icol<warp->lnd.ncols; icol++)
	    fbuf[icol] = fbuf8[icol];
	}
	else
	  fread(fbuf, warp->nbyte[k], warp->lnd.ncols, warp->fp[k]);
      }
      for(icol=0; icol<warp->lnd.ncols; icol++)
	...
	fwrite(&tmp, warp->nbyte[k], 1, temp_out);
      }
...
{
  char   temp_name[MAX_STRLEN];
  int    i, j, k, percent_done, ret, m, n, mm, nn, flag;
  int    new_ncols, new_nrows;
  short int irow, icol;
  double MinX, MaxX, MinY, MaxY, tsum, tref;
  double wx, wy, bx, by, dx, dy, v[5][5], w[5][5];    
  uint8  *fbuf8;
  int16 *fbuf, **data, tmp;
  FILE   *temp_out;
  ...
  for(k=0; k<warp->nbands; k++) {
    ...
    /* create temporary files for warp image */
    sprintf(temp_name, "%s.reprojected_warp.b%d", tempnam(".", "temp_"), k+1);
    if((temp_out=fopen(temp_name, "wb"))==NULL) {
	...
	/* convert coordinate to the center of pixel  */
	bx = MinX + (j+0.5) * warp->lnd.res;
	by = MaxY - (i+0.5) * warp->lnd.res;

	ret = convertCoor(base, &bx, &by, &(warp->proj), &wx, &wy, BACKWARD_PROJ);

	/* use cubic convolution resampling approach */
	/* compute nearest upleft pixel */
	irow = (int)((warp->lnd.uly - (wy + 0.5*warp->lnd.res))/warp->lnd.res); 
	icol = (int)(((wx - 0.5*warp->lnd.res) - warp->lnd.ulx)/warp->lnd.res);
	flag = 1;
	for(m=0; m<4; m++)       
	  for(n=0; n<4; n++) {
	    mm = irow + m -1;
	    nn = icol + n -1;
	    if(mm>=0 && mm<warp->lnd.nrows && nn>=0 && nn<warp->lnd.ncols)
	      v[m][n] = data[mm][nn];
	    else
	      flag = 0;
	
	    dx = (wx-0.5*warp->lnd.res)-warp->lnd.ulx - nn*warp->lnd.res;
	    dy = warp->lnd.uly - (wy +0.5*warp->lnd.res) - mm*warp->lnd.res;
	   
	    w[m][n] =cubic(dx/warp->lnd.res)*cubic(dy/warp->lnd.res); 
	...
	if(flag == 1) {
	  ...
	  tmp = (int)(tref/tsum+0.5);
	}
	else 
	  tmp = warp->lnd.fillValue;	

	fwrite(&tmp, warp->lnd.nbyte, 1, temp_out);  
      }
...
int reprojectDEM(DEM *sdem, BASE_LANDSAT *base)
{
  ...
  sprintf(temp_name, "%s.reprojected_dem", tempnam(".", "temp_"));
  if((temp_out=fopen(temp_name, "wb"))==NULL) {
  ...
  /* reset projection info */
  sdem->proj.sys = UTM;
  sdem->proj.utm_zone = base->proj.utm_zone;
  sdem->proj.datum = base->proj.datum;
...
int writeENVIheader(WARP_LANDSAT *warp, OUT_LANDSAT *out)
{
  ...
  for(iband=0; iband<out->nbands; iband++) {
    ...
    if(out->proj.sys == UTM) {
      ...
      if(out->proj.utm_zone > 0)
	fprintf(fp, "%d, North, WGS-84, units=Meters}\n", abs(out->proj.utm_zone));
      else
