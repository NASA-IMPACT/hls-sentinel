 ...
 * ! Revision History
 *
 *   Revision 2.3.0  4/26/2016  Feng Gao
 *   - used unique temporary filename and thus allowing multiple processes run simultaneously 
 *     in one directory
 *
 *   Revision 2.2.9  2/26/2016  Feng Gao
 *   - revised resampling approach from NN to CC while reprojection is needed for  warp image
 *     (this is an internal reprojection when base and warp images have different projections)
 *
 *   Revision 2.2.8  2/12/2016  Feng Gao
 *   - accepts base image in different projection or UTM zone (follow GCTPC)
 *
 *   Revision 2.2.7  1/28/2016  Feng Gao
 *   - added Landsat8 and Sentinel-2 parameters
 *   - accepts short integer type for both base and warp images
 *   - allows none resampling option for co-registration
 *   - allows to save final control points defined by CP_LOG_FILE  
 *
...
int main(int argc, char *argv[])
{
  ...
  if(inPars->option == REG_ONLY || inPars->option == DO_BOTH || inPars->option == VERIFY_ONLY) {
    ...
    if(base->proj.sys == UTM) 
      printf("\n\t\tutm_zone = %d; \tdatum = %ld", base->proj.utm_zone, base->proj.datum);
    else {
      printf("\n\t\tproj_code = %d;  datum = %ld;  unit=%d", base->proj.sys, base->proj.datum, base->proj.unit);
      /*printf("\n\t\tparameters[15] = ");
	for(i=0; i<15; i++)
	printf("%f ", base->proj.param[i]);*/
    }
  } else {
  ...
  if(inPars->option != VERIFY_ONLY && strcasecmp(inPars->resampleMethod, "none") !=0) {
