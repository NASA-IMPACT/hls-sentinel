...
int extractNadirPath(LANDSAT *sr)
{
  ...
  for(i=0; i<sr->nrows; i++) {
    ...
    if(strcasecmp(sr->fileType, "GEOTIFF")==0) {
      ...
      if(sr->nbyte == 1) {
	if (!TIFFReadScanline(sr->fp_tiff, buf8, i, 0)) {
	  fprintf(stderr, "Read line %d error\n", i);
	  return FAILURE;
	} 
	for(j=0; j<sr->ncols; j++) buf[j] = buf8[j];
      }
      else 
	if (!TIFFReadScanline(sr->fp_tiff, buf, i, 0)) {
      ...
      offset = (long) (i * sr->ncols * sr->nbyte);  
      /* read data array from binary file */
      fseek(sr->fp, offset, 0);
      if(sr->nbyte == 1) {
	fread(buf8, sizeof(uint8), sr->ncols, sr->fp);
	for(j=0; j<sr->ncols; j++) buf[j] = buf8[j];
      }
      else
	fread(buf, sizeof(uint16), sr->ncols, sr->fp);
    }
