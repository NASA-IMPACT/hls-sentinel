...
then
	...
	echo  "PPS10;$granbase;no_rawS10;$SUBDATE;$now"  >&2
	exit 1
fi
...
then
	...
	then
		...
		echo "PPS10;$granbaseA;error_consolidate;$SUBDATE;$now" >&2
		echo "PPS10;$granbaseB;error_consolidate;$SUBDATE;$now" >&2
		exit 1
	fi
else
	# May 8, 2017:
...
then	# No need to co-register. This is final S10.
	...
	then
		...
		echo "PPS10;$granbase;error in resamp_s2;$SUBDATE;$now"  >&2
		exit 1
	fi
fi
